    ...
    <div class="main-warp">
      <div class="main-template-begin" v-for = "item in mainData">
        ...
        <div class="main-template" v-for =" items in item.data">
          <div class="main-center-show" @click="goDetail(items.backStatus,items.monthBillId)">
            <p v-cloak>{{items.billPeriodMonth}}月记账</p>
            <p v-cloak>￥{{priceFormat(items.daihuanNum + items.overdueInterest + items.billInterest)}}</p>
            <div class="right-div">
              <span class="blue" v-if = "items.backStatus == 1">已还清</span>
              <span class="red" v-if = "items.backStatus == 0">未还清</span>
  ...
  export default({
    ...
    methods:{
      ...
      getInfo(){
        ...
        var data = {};
//        data.id = that.id;
        this.$http.post('/api/ws/bp/h5/getCarrierCarAllBill').then(function(res){
          if (res.body.code == 1 ){
